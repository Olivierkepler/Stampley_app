  ...
  return (
    <>
      <style>{`
        ...
        .badge-in { animation: badgeIn 0.35s cubic-bezier(0.22,1,0.36,1) forwards; }
      `}</style>

      <nav className="f-body relative w-full h-full py-8 px-5 flex flex-col justify-between overflow-hidden">

        {/* SUCCESS AMBIENCE: Soft teal glow when finished — adapted from project's steel-blue palette */}
        <AnimatePresence>
          {isFullyComplete && (
            <motion.div
              initial={{ opacity: 0 }}
              ...
              >
                ...
                {!collapsed && (
                  <div className="flex flex-col min-w-0 pb-0.5">
                    <span
                      className={`f-mono text-[8px] uppercase tracking-[0.2em] mb-1 transition-colors duration-300 ${
                        isActive
                          ? "text-[#3d5a80]"
                          : isCompleted || (isFinalNode && isFullyComplete)
                          ? "text-[#9d7855]"
                    ...
                    <span
                      className={`step-label f-body text-[13px] font-medium tracking-tight transition-colors duration-300 ${
                        isActive
                          ? "text-[#0a0a0f]"
          ...
          <AnimatePresence>
            {isFullyComplete && (
              <motion.div
                ...
                className="mt-8 p-4 rounded-2xl border border-[#9d7855]/20 bg-[#9d7855]/[0.04] flex items-center gap-3 backdrop-blur-sm"
              >
                {/* Icon */}
                ...
                <div className="flex flex-col min-w-0">
                  <span className="f-mono text-[8.5px] uppercase tracking-[0.2em] text-[#9d7855]/70 leading-none mb-1 select-none">
                    Status
                  </span>
